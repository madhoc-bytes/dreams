...
def test_join_invalid_uid():
    ...
    clear_v2()
    test_user = auth_register_v2("test@gmail.com", "password", "testF", "testL")['token']
    test_channel_id = channels_create_v2(test_user, "test channel", True)['channel_id']
    invalid = 10
    with pytest.raises(InputError):
        channel_join_v2(invalid, test_channel_id)

def test_join_invalid_channel_id():
    '''Passing an inavlid channel id into join'''
    clear_v2()
    test_user = auth_register_v2("test@gmail.com", "password", "testF", "testL")['auth_user_id']
    with pytest.raises(InputError):
        channel_join_v2(test_user, "invalid channel id")
...
def test_messages_nomessage():
    ...
    channel_join_v2(test_user, test_channel)
    assert channel_messages_v2(test_user, test_channel, 0) == {
        'messages': [],
        'start': 0,
        'end': -1,
    }
...
def test_messages_invalid_uid():
    ...
    invalid = 10
    with pytest.raises(InputError):
        channel_messages_v2(invalid, test_channel, 0)
...
def test_messages_invalid_ch_id():
    ...
    invalid = 10
    with pytest.raises(InputError):
        channel_messages_v2(test_user, invalid, 0)
...
def test_messages_start_too_big():
    ...
    channel_join_v2(test_user, test_channel)
    with pytest.raises(InputError):
        channel_messages_v2(test_user, test_channel, 1)

def test_messages_not_member():
    '''Call messages given a user not a member of the channel'''
    clear_v2()
    test_user = auth_register_v2("test@gmail.com", "password", "testF", "testL")['token']
    test_user2 = auth_register_v2("user@example.com", "password2", "testFF", "testLL")['token']
    test_channel = channels_create_v2(test_user, "test channel", True)['channel_id']
    channel_join_v2(test_user, test_channel)
    with pytest.raises(AccessError):
        channel_messages_v2(test_user2, test_channel, 0)
